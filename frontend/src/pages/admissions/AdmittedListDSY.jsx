...
const AdmittedListDSY = ({
  ...
  const hiddenColumnCss = columns.map((column, index) => visibleColumns.includes(column.key) ? '' : `.dsy-admissions-table th:nth-child(${index + 1}), .dsy-admissions-table td:nth-child(${index + 1}) { display: none; }`).filter(Boolean).join('\n');
  ...
  const getDocumentStatus = (admission) => {
    ...
    const totalRequired = dsyRequiredDocuments.length;
  ...
  const downloadAdmissionPdf = async (admission) => {
    ...
    try {
      await admissionService.downloadDSYAdmissionFormPdf(admission.id);
    } catch (error) {
  ...
  return (
    <div style={{
      ...
      <table className="dsy-admissions-table" style={{
        width: '100%',
        minWidth: '1040px',
